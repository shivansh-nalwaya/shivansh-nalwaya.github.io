...
for (let i = 0; i < w; i++) {
  var data = [];
  for (let j = 0; j < h; j++) {
    const dataPoint = 20 + -20 * Math.sin((i / 400) * Math.PI) * Math.sin((j / 200) * Math.PI);
    data.push(dataPoint);
  }
  hfdata.push(data);
}
...
heightfieldBody.position = new CANNON.Vec3(-200, -10, 100);
heightfieldBody.quaternion = new CANNON.Quaternion(0, 0, 0, 0).setFromAxisAngle(new CANNON.Vec3(1, 0, 0), -Math.PI / 2);
world.addBody(heightfieldBody);

const geometry = new THREE.PlaneBufferGeometry(400, 200, 256, 256);
const count = geometry.attributes.position.count;
for (var i = 0; i < count; i++) {
  const x = geometry.attributes.position.getX(i) + 200;
  const y = geometry.attributes.position.getY(i) + 100;
  const xsin = Math.sin((x / 400) * Math.PI);
  const ysin = Math.sin((y / 200) * Math.PI);
  geometry.attributes.position.setZ(i, 20 + -20 * xsin * ysin);
}
